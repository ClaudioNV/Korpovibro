...
function CambiarPass(){
    ...
    newPass = $.trim($('#password1').val());
    reconfirPass = $.trim($('#password2').val());
    repToken = $.trim($('#tokenRecepcionado').val());

    if(newPass =='')
        {
            mensage="Debes ingresar tu nueva Contraseña";
            shakeModalVerificar(mensage);
        }
    else if (reconfirPass == ''){
        mensage = "Debes Reingresar tu contraseña";
        shakeModalVerificar(mensage);

        } 
        else if (reconfirPass != newPass){
            mensage = "Las Contraseñas Deben ser iguales Intente otra vez";
            shakeModalVerificar(mensage);
    ...
    $.getJSON(repos, { op: 'upPassword', pass: newPass, tok:repToken })
        .done(function (dataPass) {

            if (dataPass.sms == "errorToken") {
                mensage = "Este token se encuentra vencido favor volver a recuperar contraseña";
            shakeModalVerificar(mensage);;
            } 
            else {
                mensage="Tu contraseña ha sido actualizada";
                shakeModalVerificarG(mensage);
