var repos = "http://korpovibro.cl/modelo/registra_todas_clases.php";
        	...
        	for (var i = 0; i < data.length; i++) {
                ...
                var idclasezumba = usuario.id;
	              ...
	              if (usuario.latitud!="") { // valido que el campo de la latitud no este vacio
                      ...
                      var value = "<center style='width:180px;'><img style ='width: 6em;' src ='" + imgtipo + "'><br><span style='font-size:1.5em;color:#da8160;'><b>" + usuario.instructor+ "</b></span><br><br> <button onclick=(funtiondatos("+idclasezumba+"))>abrir</button> </center>";
...
function funtiondatos(id) {
    ...
    window.location.assign("http://korpovibro.cl/Registro.php");
...
//alert();;
        //$('#iframe_viaje').prop('src', 'https://estoyseguro.cl/pago/mercadopagos.php?trx_id=5' + viajes_plan_emision + cotiza_viaje + '&monto=' + monto_pesos + '&prod=5&cot=' + cotiza_viaje);
       /* $('#iframe_pago').prop('src', 'http://korpovibro.cl/pago/mercadopagos.php?trx_id=5');

        $('#modalViajeCargando').modal('show');
        $('#iframe_pago').load(function () {
            $('#modalViajeCargando').modal('hide');
        });
